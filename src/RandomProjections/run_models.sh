...
if [ $SCRIPT = "baseline" ]; then
  ...
  python3 "random_ensemble.py" $DATASET_NAME $TEST $N_PROJ_LIST $SIZE_PROJ_LIST $PROJ_MODE $ATTACK $DEVICE>> $OUT
elif [ $SCRIPT = "parallel_randens" ]; then
  python3 "parallel_random_ensemble.py" $DATASET_NAME $TEST $N_PROJ $SIZE_PROJ_LIST $PROJ_MODE $DEVICE>> $OUT
